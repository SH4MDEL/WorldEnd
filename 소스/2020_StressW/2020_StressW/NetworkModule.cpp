...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_PACKET_UPDATE_CLIENT: {
		...
		if (move_packet->data->id < MAX_CLIENTS) {
			int my_id = client_map[move_packet->data->id];
			if (-1 != my_id) {
				g_clients[my_id].x = move_packet->data->pos.x;
				g_clients[my_id].y = move_packet->data->pos.y;
			}
			if (ci == my_id) {
	...
	case SC_PACKET_LOGIN_OK:
	{
		...
		int my_id = ci;
		client_map[login_packet->player_data.id] = my_id;
		g_clients[my_id].id = login_packet->player_data.id;
		g_clients[my_id].x = login_packet->player_data.pos.x;
		g_clients[my_id].y = login_packet->player_data.pos.y;
	...
	case SC_PACKET_UPDATE_MONSTER:
	{
		...
		int my_id = ci;
		g_monsters[my_id].id = monster_packet->monster_data.id;
		g_monsters[my_id].x = monster_packet->monster_data.pos.x;
		g_monsters[my_id].z = monster_packet->monster_data.pos.z;
	...
	case SC_PACKET_ADD_MONSTER:
	{
		...
		int my_id = ci;
		g_monsters[my_id].id = mon_add_packet->monster_data.id;
		g_monsters[my_id].x = mon_add_packet->monster_data.pos.x;
		g_monsters[my_id].z = mon_add_packet->monster_data.pos.z;
	}
	break;
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			CS_PLAYER_MOVE_PACKET my_packet;
			my_packet.size = sizeof(my_packet);
			my_packet.type = CS_PACKET_PLAYER_MOVE;
			switch (rand() % 2) {
			case 0: my_packet.pos.x = 0; break;
			case 1: my_packet.pos.y = 1; break;
