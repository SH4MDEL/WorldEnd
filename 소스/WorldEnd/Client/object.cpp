...
//			auto mesh = make_shared<HeightMapGridMesh>(device, commandList, xStart, zStart, blockWidth, blockLength, m_scale, m_heightMapImage.get());
//			block->SetMesh(mesh);
...
{
	// 높이맵이미지 로딩
	m_heightMapImage = make_unique<FieldMapImage>(m_width, m_length, m_height, m_scale);
	...
	for (int z = 0; z < lengthBlockCount; ++z)
		for (int x = 0; x < widthBlockCount; ++x)
		{
			...
			int zStart{ z * (blockLength - 1) };
			unique_ptr<GameObject> block{ make_unique<GameObject>() };
			auto mesh = make_shared<FieldMapGridMesh>(device, commandList, xStart, zStart, blockWidth, blockLength, m_scale, m_heightMapImage.get());
			block->SetMesh(mesh);
			m_blocks.push_back(move(block));
		}
}
...
XMFLOAT3 Field::GetNormal(FLOAT x, FLOAT z) const
{
	// 파라미터로 들어온 (x, z)는 플레이어의 위치이다.
