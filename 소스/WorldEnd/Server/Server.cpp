...
void process_packet(int c_id, char* packet)
{
	...
	case CS_MOVE: {
		...
		case INPUT_KEY_W: move_dir.x++; break;
		case INPUT_KEY_A: move_dir.z--; break;
		case INPUT_KEY_S: move_dir.x--; break;
		case INPUT_KEY_D: move_dir.z++; break;
		}
...
void WorkerThread() 
{
	for (;;) {
		...
		case OP_ACCEPT: {
			...
			AcceptEx(g_s_socket, c_socket, g_a_over._send_buf, 0, addr_size + 16, addr_size + 16, 0, &g_a_over._over);
			break;
		}
		case OP_RECV: {
			/*if (0 == num_bytes) {
