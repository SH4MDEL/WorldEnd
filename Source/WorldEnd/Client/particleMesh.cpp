...
PumperParticleMesh::PumperParticleMesh(const ComPtr<ID3D12Device>& device, const ComPtr<ID3D12GraphicsCommandList>& commandList)
{
	m_nIndices = 0;
	m_primitiveTopology = D3D_PRIMITIVE_TOPOLOGY_POINTLIST;
	m_isBinding = false;

	vector<PumperParticleVertex> vertices;
	...
	m_nVertices = (UINT)vertices.size();
	m_vertexBuffer = CreateBufferResource(device, commandList, vertices.data(),
		sizeof(PumperParticleVertex) * vertices.size(), D3D12_HEAP_TYPE_DEFAULT,
		D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, m_vertexUploadBuffer);

	m_vertexBufferView.BufferLocation = m_vertexBuffer->GetGPUVirtualAddress();
	m_vertexBufferView.StrideInBytes = sizeof(PumperParticleVertex);
	m_vertexBufferView.SizeInBytes = sizeof(PumperParticleVertex) * vertices.size();

	CreateStreamOutputBuffer(device, commandList);
}

void PumperParticleMesh::CreateStreamOutputBuffer(const ComPtr<ID3D12Device>& device, const ComPtr<ID3D12GraphicsCommandList>& commandList)
{
	ComPtr<ID3D12Resource> streamOutputUploadBuffer;
	m_streamOutputBuffer = CreateBufferResource(device, commandList, nullptr,
		sizeof(PumperParticleVertex) * MAX_PARTICLE_MESH, D3D12_HEAP_TYPE_DEFAULT,
		D3D12_RESOURCE_STATE_STREAM_OUT, streamOutputUploadBuffer);
	...
	m_streamOutputBufferView.BufferLocation = m_streamOutputBuffer->GetGPUVirtualAddress();
	m_streamOutputBufferView.SizeInBytes = sizeof(PumperParticleVertex) * MAX_PARTICLE_MESH;
	m_streamOutputBufferView.BufferFilledSizeLocation = m_filledSizeBuffer->GetGPUVirtualAddress();
...
void PumperParticleMesh::RenderStreamOutput(const ComPtr<ID3D12GraphicsCommandList>& commandList)
{
	if (!m_isBinding) {
		m_isBinding = true;
		m_vertexBufferView.BufferLocation = m_vertexBuffer->GetGPUVirtualAddress();
		m_vertexBufferView.StrideInBytes = sizeof(PumperParticleVertex);
		m_vertexBufferView.SizeInBytes = sizeof(PumperParticleVertex) * m_nVertices;
	}
	...
	*m_filledSizeUploadBufferSize = 0;
	...
	UINT64* filledSize = NULL;
	m_filledSizeReadbackBuffer->Map(0, NULL, reinterpret_cast<void**>(&filledSize));
	m_nVertices = UINT(*filledSize) / sizeof(PumperParticleVertex);
	m_filledSizeReadbackBuffer->Unmap(0, NULL);
...
void PumperParticleMesh::Render(const ComPtr<ID3D12GraphicsCommandList>& commandList)
{
	m_vertexBufferView.BufferLocation = m_drawBuffer->GetGPUVirtualAddress();
	m_vertexBufferView.StrideInBytes = sizeof(PumperParticleVertex);
	m_vertexBufferView.SizeInBytes = sizeof(PumperParticleVertex) * m_nVertices;
